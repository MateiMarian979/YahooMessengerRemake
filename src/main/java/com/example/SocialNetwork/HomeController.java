...
public class HomeController implements Observer<Message>{
    ...
    public void initialize(){
        messageEmailCol.setCellValueFactory(new PropertyValueFactory<>("fromUser"));
        messageTextCol.setCellValueFactory(new PropertyValueFactory<>("message"));
    ...
    public void handleSelectMessage(){
        selectedMessage = messageTableView.getSelectionModel().getSelectedItem();
    }
    ...
    private void loadMessages() {
        try {
            ...
            for(Message m : messageIterable){
                if(m.getReplyID() != 0){
                    m.setMessage("Reply " + messageService.getMessageContent(m.getReplyID()) + " :" + m.getMessage());
                }
            }
            List<Message> messagesList = StreamSupport.stream(messageIterable.spliterator(), false)
                    .collect(Collectors.toList());
